...
class SimpleMarketUI:
    """🚀 ALL-TIME BEST MARKET OVERVIEW - INSTITUTIONAL GRADE INTELLIGENCE 🚀
    
    Revolutionary filtered market analysis providing:
    - Real-time filtered set statistics and performance metrics
    - Smart alerts for opportunities within your selection
    - Comparison intelligence vs full market baseline
    - Top opportunities highlight with institutional-grade insights
    - Multi-dimensional filtered universe analysis
    
    Designed for rapid decision-making on filtered stock selections while
    maintaining V9.py patterns for session state, error handling, and UX.
    """
    ...
    def render_market_analysis(df: pd.DataFrame):
        ...
        try:
            ...
            # ====================================
            # TIER 2: FILTERED SET INTELLIGENCE
            # ====================================
            st.markdown("---")
            st.markdown("### 📊 **Your Filtered Selection Intelligence**")
            st.markdown("*Real-time metrics and insights for your filtered stock universe*")
            ...
            try:
                ...
                with stats_cols[0]:
                    if 'master_score' in df.columns:
                        ...
                        score_quality = "🔥 Elite" if avg_score >= 80 else "📈 Strong" if avg_score >= 70 else "⚖️ Average" if avg_score >= 60 else "📉 Weak"
                        ...
                        UIComponents.render_metric_card("Avg Master Score", "N/A")
                ...
                with stats_cols[1]:
                    if 'ret_30d' in df.columns:
                        ...
                        if len(valid_returns) > 0:
                            ...
                            win_quality = "🚀 Excellent" if win_rate >= 70 else "✅ Good" if win_rate >= 60 else "⚖️ Mixed" if win_rate >= 50 else "⚠️ Weak"
                            UIComponents.render_metric_card(
                                "30D Win Rate",
                                f"{win_rate:.1f}%",
                                win_quality,
                                help_text="Percentage of stocks with positive 30-day returns"
                            )
                        else:
                            UIComponents.render_metric_card("30D Win Rate", "N/A")
                    else:
                        UIComponents.render_metric_card("30D Win Rate", "N/A")
                
                with stats_cols[2]:
                    if 'master_score' in df.columns:
                        elite_count = (df['master_score'] >= 80).sum()
                        elite_pct = (elite_count / len(df)) * 100 if len(df) > 0 else 0
                        elite_quality = "💎 Exceptional" if elite_pct >= 30 else "🏆 High Quality" if elite_pct >= 20 else "📊 Moderate" if elite_pct >= 10 else "📉 Limited"
                        UIComponents.render_metric_card(
                            "Elite Stocks %",
                            f"{elite_pct:.1f}%",
                            f"{elite_count} stocks",
                            help_text="Percentage of stocks with score >= 80"
                        )
                    else:
                        UIComponents.render_metric_card("Elite Stocks %", "N/A")
                
                with stats_cols[3]:
                    if 'rvol' in df.columns:
                        valid_rvol = df['rvol'].dropna()
                        if len(valid_rvol) > 0:
                            avg_rvol = valid_rvol.mean()
                            rvol_quality = "🌊🌊 Massive" if avg_rvol >= 3 else "🌊 High" if avg_rvol >= 2 else "💧 Normal" if avg_rvol >= 1.5 else "🏜️ Low"
                            high_vol_count = (valid_rvol >= 2).sum()
                            UIComponents.render_metric_card(
                                "Avg Rel Volume",
                                f"{avg_rvol:.2f}x",
                                f"{rvol_quality} • {high_vol_count} hot",
                                help_text="Average relative volume (2x+ is high activity)"
                            )
                        else:
                            UIComponents.render_metric_card("Avg Rel Volume", "N/A")
                    else:
                        UIComponents.render_metric_card("Avg Rel Volume", "N/A")
                        
            except Exception as e:
                logger.error(f"Filtered set statistics failed: {e}", exc_info=True)
                st.warning("⚠️ Unable to calculate filtered set statistics")
            
            # ====================================
            # TIER 3: SMART ALERTS & OPPORTUNITIES
            # ====================================
            st.markdown("---")
            st.markdown("### 🚨 **Smart Alerts in Your Selection**")
            ...
            try:
                ...
                with alert_col1:
                    ...
                    if 'from_high_pct' in df.columns:
                        near_high = df[(df['from_high_pct'] > -5) & (df['from_high_pct'] <= 0)]
                        if len(near_high) > 0:
                            st.success(f"✅ **{len(near_high)} stocks** within 5% of 52W high!")
                            
                            # Show top 3
                            top_near_high = near_high.nsmallest(3, 'from_high_pct')
                            for _, stock in top_near_high.iterrows():
                                ticker = stock.get('ticker', 'N/A')
                                distance = stock.get('from_high_pct', 0)
                                score = stock.get('master_score', 0)
                                st.caption(f"🎯 **{ticker}**: {distance:.1f}% away (Score: {score:.0f})")
                        else:
                            st.info("No stocks near 52W high in selection")
                    else:
                        st.info("52W high data unavailable")
                
                with alert_col2:
                    st.markdown("**🌊 Unusual Volume**")
                    if 'rvol' in df.columns:
                        high_volume = df[df['rvol'] >= 3.0]
                        if len(high_volume) > 0:
                            st.success(f"✅ **{len(high_volume)} stocks** with 3x+ volume!")
                            
                            # Show top 3 by RVOL
                            top_volume = high_volume.nlargest(3, 'rvol')
                            for _, stock in top_volume.iterrows():
                                ticker = stock.get('ticker', 'N/A')
                                rvol = stock.get('rvol', 0)
                                score = stock.get('master_score', 0)
                                st.caption(f"🎯 **{ticker}**: {rvol:.1f}x volume (Score: {score:.0f})")
                        else:
                            st.info("No unusual volume detected")
                    else:
                        st.info("Volume data unavailable")
                
                with alert_col3:
                    st.markdown("**🚀 Momentum Surge**")
                    if 'momentum_score' in df.columns and 'acceleration_score' in df.columns:
                        momentum_surge = df[
                            (df['momentum_score'] >= 75) & 
                            (df['acceleration_score'] >= 75)
                        ]
                        if len(momentum_surge) > 0:
                            st.success(f"✅ **{len(momentum_surge)} stocks** with strong momentum!")
                            
                            # Show top 3 by master score
                            top_momentum = momentum_surge.nlargest(3, 'master_score')
                            for _, stock in top_momentum.iterrows():
                                ticker = stock.get('ticker', 'N/A')
                                mom = stock.get('momentum_score', 0)
                                accel = stock.get('acceleration_score', 0)
                                st.caption(f"🎯 **{ticker}**: M:{mom:.0f} A:{accel:.0f}")
                        else:
                            st.info("No momentum surge detected")
                    else:
                        st.info("Momentum data unavailable")
                        
            except Exception as e:
                logger.error(f"Smart alerts failed: {e}", exc_info=True)
                st.warning("⚠️ Unable to generate smart alerts")
            
            # ====================================
            # TIER 4: TOP OPPORTUNITIES HIGHLIGHT
            # ====================================
            st.markdown("---")
            st.markdown("### 🎯 **Top 3 Opportunities in Your Selection**")
            
            try:
                if 'master_score' in df.columns and len(df) > 0:
                    top_3 = df.nlargest(3, 'master_score')
                    
                    opp_cols = st.columns(3)
                    
                    for idx, (col, (_, stock)) in enumerate(zip(opp_cols, top_3.iterrows())):
                        with col:
                            # Rank medal
                            medals = ["🥇", "🥈", "🥉"]
                            ...
                            st.markdown(f"**{medal} #{idx+1}: {ticker}**")
                            ...
                            if 'price' in stock.index:
                                metrics_to_show.append(f"Price: ₹{stock['price']:,.0f}")
                            
                            if 'ret_30d' in stock.index and pd.notna(stock['ret_30d']):
                                ret_30d = stock['ret_30d']
                                ret_color = "🟢" if ret_30d > 0 else "🔴"
                                metrics_to_show.append(f"{ret_color} 30D: {ret_30d:+.1f}%")
                            
                            if 'rvol' in stock.index and pd.notna(stock['rvol']):
                                rvol = stock['rvol']
                                vol_emoji = "🌊" if rvol >= 2 else "💧"
                                metrics_to_show.append(f"{vol_emoji} RVOL: {rvol:.1f}x")
                            
                            if 'from_high_pct' in stock.index and pd.notna(stock['from_high_pct']):
                                from_high = stock['from_high_pct']
                                metrics_to_show.append(f"52WH: {from_high:.1f}%")
                            ...
                            score_color = "🔥" if score >= 85 else "📈" if score >= 75 else "⚖️"
                            st.metric("Master Score", f"{score:.1f}", f"{score_color}")
                            
                            # Pattern info if available
                            if 'patterns' in stock.index and pd.notna(stock['patterns']) and stock['patterns']:
                                patterns = stock['patterns'].split(' | ')
                                if patterns:
                                    first_pattern = patterns[0][:30] + "..." if len(patterns[0]) > 30 else patterns[0]
                                    st.caption(f"🎯 {first_pattern}")
                else:
                    st.info("No stocks in filtered selection")
                    
            except Exception as e:
                logger.error(f"Top opportunities highlight failed: {e}", exc_info=True)
                st.warning("⚠️ Unable to generate top opportunities")
