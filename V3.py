...
class RankingEngine:
    ...
    def _calculate_position_score(df: pd.DataFrame) -> pd.Series:
        ...
        USES PERCENTILE RANKING - This is INTENTIONAL for position scoring because
        position is inherently relative. We want to know which stocks are BEST positioned
        compared to others, not their absolute position.
        
        Position Score Logic:
        - 60% weight: Distance from 52w low (percentile rank)
        - 40% weight: Distance from 52w high (percentile rank)
        - Special handling for stocks above 52w high
        - Smart bonuses/penalties based on position zones
        """
        ...
        # For from_high: Need to handle both negative (below high) and positive (above high)
        # Create adjusted series for ranking
        from_high_adjusted = pd.Series(np.nan, index=df.index, dtype=float)
        
        # Below 52w high (negative values): Convert to distance
        below_high = valid_mask & (from_high <= 0)
        from_high_adjusted[below_high] = from_high[below_high].abs()  # Smaller is better
        
        # Above 52w high (positive values): Penalize overextension
        above_high = valid_mask & (from_high > 0)
        if above_high.any():
            # Invert and add penalty for overextension
            # +5% above = good, +30% above = careful, +50% = bad
            from_high_adjusted[above_high] = from_high[above_high] * 2  # Double weight for overextension
        
        # Rank with smaller distance being better
        rank_from_high = RankingEngine._safe_rank(from_high_adjusted, pct=True, ascending=False)
        ...
        # APPLY MODIFIERS - These are good, keep them!
        ...
        # 2. Overextended penalty: >85% from low (but not breaking out)
        overextended = valid_mask & (from_low > 85) & (from_high <= 0)
        if overextended.any():
            position_score.loc[overextended] *= 0.90
            logger.debug(f"Applied overextended penalty (-10%) to {overextended.sum()} stocks")
        
        # 3. Near 52w high bonus: Within 5% of high (approaching breakout)
        near_high = valid_mask & (from_high > -5) & (from_high <= 0)
        if near_high.any():
            position_score.loc[near_high] *= 1.05
            logger.debug(f"Applied near-high bonus (+5%) to {near_high.sum()} stocks")
        
        # 4. Fresh breakout bonus: Just crossed above high
        fresh_breakout = valid_mask & (from_high > 0) & (from_high <= 5)
        if fresh_breakout.any():
            position_score.loc[fresh_breakout] *= 1.15
            logger.debug(f"Applied fresh breakout bonus (+15%) to {fresh_breakout.sum()} stocks")
        
        # 5. Extreme breakout penalty: Too far above high
        extreme_breakout = valid_mask & (from_high > 30)
        if extreme_breakout.any():
            position_score.loc[extreme_breakout] *= 0.85
            logger.debug(f"Applied extreme breakout penalty (-15%) to {extreme_breakout.sum()} stocks")
        
        # 6. Near 52w low penalty: Within 10% of low
        ...
        if 'rvol' in df.columns and above_high.any():
            rvol = pd.Series(df['rvol'].values, index=df.index)
            
            # High volume breakout = more reliable
            volume_breakout = above_high & (rvol > 2)
            if volume_breakout.any():
                position_score.loc[volume_breakout] *= 1.05
                logger.debug(f"Applied volume breakout bonus (+5%) to {volume_breakout.sum()} stocks")
            
            # Low volume breakout = suspicious
            weak_breakout = above_high & (from_high <= 10) & (rvol < 1)
            if weak_breakout.any():
                position_score.loc[weak_breakout] *= 0.90
                logger.debug(f"Applied weak breakout penalty (-10%) to {weak_breakout.sum()} stocks")
        
        # CATEGORY ADJUSTMENTS
        if 'category' in df.columns and position_score.notna().any():
            category = pd.Series(df['category'].values, index=df.index)
            
            # Large caps breaking out are more reliable
            ...
            if large_breakout.any():
                ...
                logger.debug(f"Applied large cap breakout bonus (+5%) to {large_breakout.sum()} stocks")
            
            # Small caps near lows are riskier
            is_small = category.isin(['Micro Cap', 'Small Cap'])
            small_near_low = is_small & valid_mask & (from_low < 20)
            if small_near_low.any():
                position_score.loc[small_near_low] *= 0.95
                logger.debug(f"Applied small cap risk penalty (-5%) to {small_near_low.sum()} stocks")
        ...
        if valid_count > 0:
            ...
            # Note about percentile ranking
            if avg_score > 45 and avg_score < 55:
                logger.debug("Average near 50 is expected with percentile ranking")
            
            # Log breakout statistics
            if above_high.any():
                breakout_count = above_high.sum()
                breakout_scores = position_score[above_high]
                logger.info(f"Breakout stocks: {breakout_count}, "
                           f"Avg score: {breakout_scores.mean():.1f}")
    ...
    def _calculate_volume_score(df: pd.DataFrame) -> pd.Series:
        """
        Calculate volume score using SIMPLE ABSOLUTE SCORING.
        FIXED: No percentile ranking, just direct ratio-to-score conversion.
        """
        # Start with NaN (honest about missing data)
        ...
        # Calculate weighted score using ABSOLUTE VALUES
        ...
        for col, weight in vol_cols:
            if col in df.columns:
                ...
                # SIMPLE FORMULA: Convert ratio directly to score
                # ratio = 1.0 → score = 50 (normal)
                # ratio = 2.0 → score = 75 (elevated)
                # ratio = 0.5 → score = 25 (low)
                
                # Linear scaling: score = 50 + (ratio - 1) * 25
                # Capped at 0-100
                col_score = (50 + (col_data - 1) * 25).clip(0, 100)
                
                weighted_sum[valid] += col_score[valid] * weight
                weight_sum[valid] += weight
        
        # Calculate final score where we have data
    ...
    def _calculate_momentum_score(df: pd.DataFrame) -> pd.Series:
        """
        Calculate momentum score based on returns using ABSOLUTE SCORING.
        FIXED: No more percentile ranking that artificially spreads similar values.
        
        Momentum Score Logic:
        - Primary: 30-day returns (absolute scoring)
        - Fallback: 7-day returns if 30d unavailable
        - Base formula: 50 + (return% * multiplier)
        - Consistency bonus: +5 if both 7d & 30d positive
        - Acceleration bonus: +10 if 7d pace > 30d pace * 1.2
        - Decay penalty: -10% if momentum is slowing progressively
        
        Score mapping:
        - 0% return = 50 score (neutral)
        - +10% return = 60 score
        - +30% return = 80 score
        - -10% return = 40 score
        - -30% return = 20 score
        ...
        # FIXED: Use ABSOLUTE SCORING instead of percentile ranking
        if has_30d:
            ret_30d = pd.Series(df['ret_30d'].values, index=df.index)
            valid_30d = ret_30d.notna()
            
            # ABSOLUTE FORMULA: Linear scaling based on actual returns
            # 30-day returns typically range from -50% to +100%
            # We'll use a multiplier of 1.0 for moderate sensitivity
            # This gives: 0% = 50, 10% = 60, 30% = 80, -10% = 40
            momentum_score[valid_30d] = (50 + ret_30d[valid_30d] * 1.0).clip(0, 100)
            
            logger.info(f"Calculated momentum scores for {valid_30d.sum()} stocks using 30-day returns")
            
            # Log statistics for debugging
            if valid_30d.any():
                avg_return = ret_30d[valid_30d].mean()
                avg_score = momentum_score[valid_30d].mean()
                logger.debug(f"Average 30d return: {avg_return:.1f}%, Average momentum score: {avg_score:.1f}")
        
        # Fallback to 7-day returns for stocks without 30-day data
        if has_7d:
            ret_7d = pd.Series(df['ret_7d'].values, index=df.index)
            needs_7d = momentum_score.isna() & ret_7d.notna()
            
            if needs_7d.any():
                # 7-day returns need higher multiplier since they're smaller
                # Use 3x multiplier so 3.33% weekly = 60 score (equivalent to 10% monthly)
                momentum_score[needs_7d] = (50 + ret_7d[needs_7d] * 3.0).clip(0, 100)
                logger.info(f"Used 7-day returns for {needs_7d.sum()} stocks as fallback")
        
        # CONSISTENCY BONUS: Reward stocks with positive momentum across timeframes
        if has_7d and has_30d and momentum_score.notna().any():
            ret_7d = pd.Series(df['ret_7d'].values, index=df.index)
            ret_30d = pd.Series(df['ret_30d'].values, index=df.index)
            
            # Only apply bonus where we have valid momentum score and both returns
            valid_for_bonus = momentum_score.notna() & ret_7d.notna() & ret_30d.notna()
            
            if valid_for_bonus.any():
                # Both positive = consistent uptrend
                consistent_positive = valid_for_bonus & (ret_7d > 0) & (ret_30d > 0)
                if consistent_positive.any():
                    momentum_score.loc[consistent_positive] += 5
                    logger.debug(f"Applied consistency bonus (+5) to {consistent_positive.sum()} stocks")
                
                # ACCELERATION BONUS: 7-day pace faster than 30-day pace
                with np.errstate(divide='ignore', invalid='ignore'):
                    daily_7d = ret_7d / 7
                    daily_30d = ret_30d / 30
                
                # Strong acceleration (7d pace > 30d pace * 1.2)
                strong_accel = valid_for_bonus & consistent_positive & (daily_7d > daily_30d * 1.2)
                if strong_accel.any():
                    momentum_score.loc[strong_accel] += 5  # Additional 5 (total +10 with consistency)
                    logger.debug(f"Applied acceleration bonus (+5 additional) to {strong_accel.sum()} stocks")
                
                # Moderate acceleration (7d pace > 30d pace * 1.1)
                moderate_accel = valid_for_bonus & consistent_positive & ~strong_accel & (daily_7d > daily_30d * 1.1)
                if moderate_accel.any():
                    momentum_score.loc[moderate_accel] += 2  # Smaller bonus
                    logger.debug(f"Applied moderate acceleration bonus (+2) to {moderate_accel.sum()} stocks")
        
        # DECAY PENALTY: Penalize slowing momentum
        if all(col in df.columns for col in ['ret_1d', 'ret_3d', 'ret_7d']) and momentum_score.notna().any():
            ret_1d = pd.Series(df['ret_1d'].values, index=df.index)
            ret_3d = pd.Series(df['ret_3d'].values, index=df.index)
            ret_7d = pd.Series(df['ret_7d'].values, index=df.index)
            
            # Only apply where we have all data and a valid score
            valid_for_decay = (
            ...
            if valid_for_decay.any():
                ...
                # Detect progressive slowdown
                slowing = valid_for_decay & (
                    (daily_pace_1d < daily_pace_3d * 0.8) &  # Today 20% slower than 3-day avg
                    (daily_pace_3d < daily_pace_7d * 0.9)     # 3-day avg 10% slower than 7-day avg
                )
                
                if slowing.any():
                    momentum_score.loc[slowing] *= 0.90  # 10% penalty
                    logger.debug(f"Applied decay penalty (-10%) to {slowing.sum()} stocks")
                
                # Severe slowdown (momentum reversal)
                reversing = valid_for_decay & (
                    (ret_1d < 0) &  # Negative today
                    (ret_7d > 5)    # But was strongly positive over week
                )
                
                if reversing.any():
                    momentum_score.loc[reversing] *= 0.85  # 15% penalty
                    logger.debug(f"Applied reversal penalty (-15%) to {reversing.sum()} stocks")
        
        # CONTEXT ADJUSTMENTS based on market conditions
        if 'category' in df.columns and momentum_score.notna().any():
            category = pd.Series(df['category'].values, index=df.index)
            
            # Small/Micro caps with extreme momentum might be pumps
            is_penny = category.isin(['Micro Cap', 'Small Cap'])
            if has_30d:
                ret_30d = pd.Series(df['ret_30d'].values, index=df.index)
                extreme_penny = is_penny & momentum_score.notna() & (ret_30d > 50)
                if extreme_penny.any():
                    momentum_score.loc[extreme_penny] *= 0.90  # Reduce by 10%
                    logger.debug(f"Applied penny stock extreme momentum penalty to {extreme_penny.sum()} stocks")
            
            # Large caps with strong momentum are more significant
            is_large = category.isin(['Large Cap', 'Mega Cap'])
            strong_large = is_large & momentum_score.notna() & (momentum_score > 70)
            if strong_large.any():
                momentum_score.loc[strong_large] *= 1.05  # Boost by 5%
        ...
        if valid_scores > 0:
            ...
            logger.info(f"Score distribution - Min: {momentum_score.min():.1f}, "
                       f"Max: {momentum_score.max():.1f}, "
                       f"Mean: {momentum_score.mean():.1f}, "
                       f"Median: {momentum_score.median():.1f}")
    ...
    def _calculate_acceleration_score(df: pd.DataFrame) -> pd.Series:
        ...
        FIXED: Uses ABSOLUTE scoring based on actual acceleration ratios, not percentiles.
        
        Acceleration Logic:
        - Compares daily return rates across timeframes
        - Positive acceleration: Short-term returns > Long-term returns
        - Negative acceleration: Short-term returns < Long-term returns
        
        Score Formula:
        - Acceleration ratio 1.0 = 50 (no change in pace)
        - Acceleration ratio 2.0 = 75 (doubling pace)
        - Acceleration ratio 0.5 = 25 (halving pace)
        """
        acceleration_score = pd.Series(np.nan, index=df.index, dtype=float)
        
        # Check what return columns we have
        ret_cols = ['ret_1d', 'ret_3d', 'ret_7d', 'ret_30d']
        available_cols = [col for col in ret_cols if col in df.columns]
        
        if len(available_cols) < 2:
            logger.warning(f"Insufficient return data for acceleration (need 2+, have {len(available_cols)})")
            return acceleration_score
        
        # Get return data
        ret_1d = pd.Series(df['ret_1d'].values, index=df.index) if 'ret_1d' in df.columns else None
        ret_3d = pd.Series(df['ret_3d'].values, index=df.index) if 'ret_3d' in df.columns else None
        ret_7d = pd.Series(df['ret_7d'].values, index=df.index) if 'ret_7d' in df.columns else None
        ret_30d = pd.Series(df['ret_30d'].values, index=df.index) if 'ret_30d' in df.columns else None
        
        # Calculate daily equivalent returns (annualized daily rate)
        daily_1d = ret_1d if ret_1d is not None else None
        daily_3d = ret_3d / 3 if ret_3d is not None else None
        daily_7d = ret_7d / 7 if ret_7d is not None else None
        daily_30d = ret_30d / 30 if ret_30d is not None else None
        
        # MAIN CALCULATION: Compare short vs long timeframes
        
        # Priority 1: 7d vs 30d acceleration (most reliable)
        if ret_7d is not None and ret_30d is not None:
            valid_mask = ret_7d.notna() & ret_30d.notna()
            
            if valid_mask.any():
                # Calculate acceleration ratio
                # Avoid division by zero and handle sign changes
                accel_ratio = pd.Series(1.0, index=df.index, dtype=float)
                ...
                for idx in df.index[valid_mask]:
                    ...
                    if abs(d30) < 0.1:  # Near zero base
                        if abs(d7) < 0.1:
                            accel_ratio[idx] = 1.0  # Both near zero = no acceleration
                        else:
                            accel_ratio[idx] = 1.5 if d7 > 0 else 0.5  # Starting to move
                    elif d30 > 0:  # Positive base
                        if d7 >= 0:
                            accel_ratio[idx] = 1 + (d7 - d30) / abs(d30)  # Normal acceleration
                        else:
                            accel_ratio[idx] = 0.2  # Reversal from positive to negative
                    else:  # Negative base
                        if d7 <= 0:
                            accel_ratio[idx] = 1 + (d30 - d7) / abs(d30)  # Less negative = acceleration
                        else:
                            accel_ratio[idx] = 2.0  # Recovery from negative to positive
                
                # Convert ratio to score (1.0 = 50, 2.0 = 75, 0.5 = 25)
                # Use bounded linear scaling
                acceleration_score[valid_mask] = (50 + (accel_ratio[valid_mask] - 1) * 25).clip(0, 100)
                
                logger.debug(f"Calculated 7d/30d acceleration for {valid_mask.sum()} stocks")
        
        # Priority 2: 1d vs 7d acceleration (for stocks without 30d data)
        if ret_1d is not None and ret_7d is not None:
            needs_calc = acceleration_score.isna() & ret_1d.notna() & ret_7d.notna()
            
            if needs_calc.any():
                accel_ratio = pd.Series(1.0, index=df.index, dtype=float)
                
                for idx in df.index[needs_calc]:
                    d1 = daily_1d[idx]
                    d7 = daily_7d[idx]
                    
                    if abs(d7) < 0.1:  # Near zero base
                        if abs(d1) < 0.5:  # Small daily move
                            accel_ratio[idx] = 1.0
                        else:
                            accel_ratio[idx] = 1.3 if d1 > 0 else 0.7  # Sudden move
                    elif d7 > 0:  # Positive base
                        if d1 >= 0:
                            # Cap extreme daily spikes
                            accel_ratio[idx] = min(1 + (d1 - d7) / abs(d7), 3.0)
                        else:
                            accel_ratio[idx] = 0.3  # Bad reversal
                    else:  # Negative base
                        if d1 <= 0:
                            accel_ratio[idx] = 1 + (d7 - d1) / abs(d7)
                        else:
                            accel_ratio[idx] = 1.8  # Good reversal
                
                # Higher volatility in 1d comparison, so compress the range
                acceleration_score[needs_calc] = (50 + (accel_ratio[needs_calc] - 1) * 20).clip(0, 100)
                
                logger.debug(f"Calculated 1d/7d acceleration for {needs_calc.sum()} stocks")
        
        # Priority 3: 3d vs 7d acceleration (fallback)
        if ret_3d is not None and ret_7d is not None:
            needs_calc = acceleration_score.isna() & ret_3d.notna() & ret_7d.notna()
            ...
            if needs_calc.any():
                ...
                for idx in df.index[needs_calc]:
                    ...
                    if abs(d7) < 0.1:
                        accel_ratio[idx] = 1.0 if abs(d3) < 0.1 else (1.2 if d3 > 0 else 0.8)
                    else:
                        accel_ratio[idx] = 1 + (d3 - d7) / abs(d7)
                
                acceleration_score[needs_calc] = (50 + (accel_ratio[needs_calc] - 1) * 22).clip(0, 100)
                
                logger.debug(f"Calculated 3d/7d acceleration for {needs_calc.sum()} stocks")
        ...
        if all(col in df.columns for col in ['ret_1d', 'ret_7d', 'ret_30d']):
            ...
            # All positive and accelerating = strong trend
            strong_accel = (
            ...
            if strong_accel.any():
                acceleration_score.loc[strong_accel] *= 1.10  # 10% bonus
                logger.debug(f"Applied consistency bonus to {strong_accel.sum()} stocks")
            
            # Deceleration warning (slowing down)
            slowing = (
                acceleration_score.notna() &
                (ret_30d > 10) &  # Was doing well
                (ret_7d < ret_30d / 4) &  # But 7d much weaker
                (ret_1d < 0)  # And today negative
            )
            if slowing.any():
                acceleration_score.loc[slowing] *= 0.85  # 15% penalty
                logger.debug(f"Applied deceleration penalty to {slowing.sum()} stocks")
        
        # EXTREME VALUE HANDLING
        if acceleration_score.notna().any():
            # Cap extreme accelerations (likely unsustainable)
            extreme_accel = acceleration_score > 90
            if extreme_accel.any():
                acceleration_score.loc[extreme_accel] = 90
                logger.debug(f"Capped {extreme_accel.sum()} extreme acceleration scores")
            
            # Floor extreme decelerations
            extreme_decel = acceleration_score < 10
            if extreme_decel.any():
                acceleration_score.loc[extreme_decel] = 10
                logger.debug(f"Floored {extreme_decel.sum()} extreme deceleration scores")
        
        # VOLUME CONFIRMATION (acceleration with volume is more reliable)
        if 'rvol' in df.columns and acceleration_score.notna().any():
            rvol = pd.Series(df['rvol'].values, index=df.index)
            
            # High acceleration with high volume = very bullish
            volume_accel = (
                acceleration_score.notna() &
            ...
            if volume_accel.any():
                acceleration_score.loc[volume_accel] *= 1.05  # 5% bonus
                logger.debug(f"Applied volume acceleration bonus to {volume_accel.sum()} stocks")
        
        # Final clipping
        acceleration_score = acceleration_score.clip(0, 100)
        
        # LOGGING
        valid_count = acceleration_score.notna().sum()
        if valid_count > 0:
            logger.info(f"Acceleration scores calculated: {valid_count} valid out of {len(df)} stocks")
            
            avg_score = acceleration_score[acceleration_score.notna()].mean()
            median_score = acceleration_score[acceleration_score.notna()].median()
            
            logger.info(f"Score distribution - Min: {acceleration_score.min():.1f}, "
                       f"Max: {acceleration_score.max():.1f}, "
                       f"Mean: {avg_score:.1f}, "
                       f"Median: {median_score:.1f}")
            ...
            logger.debug(f"Accelerating: {accelerating}, Decelerating: {decelerating}")
    ...
    def _calculate_breakout_score(df: pd.DataFrame) -> pd.Series:
        ...
        FIXED: Proper NaN handling, no fake data creation.
        
        Breakout Score Components:
        - 40% weight: Distance from 52w high (closer = higher score)
        - 40% weight: Volume surge (7d/90d ratio)
        - 20% weight: Trend support (price above SMAs)
        - Bonus: +10 if momentum confirmed (7d & 30d positive)
        """
        breakout_score = pd.Series(np.nan, index=df.index, dtype=float)
        
        # Factor 1: Distance from high (40% weight)
        distance_factor = pd.Series(np.nan, index=df.index, dtype=float)
        if 'from_high_pct' in df.columns:
            from_high = pd.Series(df['from_high_pct'].values, index=df.index)
            # DON'T fillna(-50)! Keep NaN as NaN
            
            # from_high is negative (e.g., -5 means 5% below high)
            # Convert to positive distance, then to score
            # -1% from high = 99 score, -50% from high = 50 score
            valid_high = from_high.notna()
            if valid_high.any():
                # Closer to high = better score
                distance_from_high = -from_high[valid_high]  # Make positive
                distance_factor[valid_high] = (100 - distance_from_high).clip(0, 100)
        
        # Factor 2: Volume surge (40% weight)
        volume_factor = pd.Series(np.nan, index=df.index, dtype=float)
        if 'vol_ratio_7d_90d' in df.columns:
            vol_ratio = pd.Series(df['vol_ratio_7d_90d'].values, index=df.index)
            # DON'T fillna(1.0)! Keep NaN as NaN
            
            valid_vol = vol_ratio.notna()
            if valid_vol.any():
                # Convert ratio to score (1.0 = 0, 2.0 = 100, 3.0+ = 100)
                volume_factor[valid_vol] = ((vol_ratio[valid_vol] - 1) * 100).clip(0, 100)
        ...
        if 'price' in df.columns:
            ...
            price_valid = current_price.notna()
            
            if price_valid.any():
                trend_points = pd.Series(0, index=df.index, dtype=float)
                sma_count = 0
                
                # Check each SMA (33.33 points each)
                for sma_col, points in [('sma_20d', 33.33), ('sma_50d', 33.33), ('sma_200d', 33.34)]:
                    if sma_col in df.columns:
                        ...
                        # Only check where both price and SMA exist
                        check_mask = price_valid & sma_values.notna()
                        if check_mask.any():
                            above_sma = check_mask & (current_price > sma_values)
                            trend_points[above_sma] += points
                            sma_count += 1
                
                # Only assign trend factor if we had at least one SMA
                if sma_count > 0:
                    trend_factor[price_valid] = trend_points[price_valid]
        
        # Combine factors with adaptive weighting
        # Only calculate where we have at least one factor
        has_distance = distance_factor.notna()
        has_volume = volume_factor.notna()
        has_trend = trend_factor.notna()
        
        # Track which stocks have data for scoring
        has_any_factor = has_distance | has_volume | has_trend
        
        if has_any_factor.any():
            for idx in df.index[has_any_factor]:
                factors = []
                weights = []
                
                # Add available factors
                if has_distance[idx]:
                    factors.append(distance_factor[idx])
                    weights.append(0.4)
                
                if has_volume[idx]:
                    factors.append(volume_factor[idx])
                    weights.append(0.4)
                
                if has_trend[idx]:
                    factors.append(trend_factor[idx])
                    weights.append(0.2)
                ...
                if factors and weights:
                    ...
                    if total_weight > 0:
                        ...
                        breakout_score[idx] = weighted_sum / total_weight
        
        # Momentum confirmation bonus (only where breakout score exists)
        if all(col in df.columns for col in ['ret_7d', 'ret_30d']) and breakout_score.notna().any():
            ret_7d = pd.Series(df['ret_7d'].values, index=df.index)
            ret_30d = pd.Series(df['ret_30d'].values, index=df.index)
            
            # Only apply bonus where all data exists
            momentum_confirm = (
            ...
            if momentum_confirm.any():
                breakout_score[momentum_confirm] += 10
                logger.debug(f"Applied momentum confirmation bonus to {momentum_confirm.sum()} stocks")
        
        # Additional breakout signals
        if breakout_score.notna().any():
            # Near 52w high with volume = strong breakout potential
            if 'from_high_pct' in df.columns and 'rvol' in df.columns:
                ...
                imminent_breakout = (
                    ...
                    from_high.notna() &
                    rvol.notna() &
                    (from_high > -3) &  # Within 3% of high
                    (rvol > 2)  # With strong volume
                )
                
                if imminent_breakout.any():
                    breakout_score[imminent_breakout] += 5
                    logger.debug(f"Applied imminent breakout bonus to {imminent_breakout.sum()} stocks")
        
        # Ensure scores stay within bounds
        breakout_score = breakout_score.clip(0, 100)
        
        # Log statistics
        valid_count = breakout_score.notna().sum()
        logger.info(f"Breakout scores calculated: {valid_count} valid out of {len(df)} stocks")
        
        if valid_count > 0:
            # Log factor availability
            logger.debug(f"Distance factor: {has_distance.sum()} stocks")
            logger.debug(f"Volume factor: {has_volume.sum()} stocks")
            logger.debug(f"Trend factor: {has_trend.sum()} stocks")
            
            avg_score = breakout_score[breakout_score.notna()].mean()
            high_breakout = (breakout_score > 80).sum()
            logger.debug(f"Average breakout score: {avg_score:.1f}, High potential: {high_breakout}")
